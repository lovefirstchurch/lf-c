...
import { 
  initDb, run, get, all, addAuditLog, transaction 
} from '@/lib/database';
...
export async function GET(
  ...
  try {
    ...
    // Run audit logging after response starts
    if (user) {
...
export async function POST(
  ...
  try {
    ...
    if (segments.length === 4 && segments[0] === 'arrivals' && segments[2] === 'named-attendance') {
      ...
      const { ticks } = body; // Array of { member_id, present }
